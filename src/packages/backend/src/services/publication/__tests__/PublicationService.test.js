...
const createConnector = ({ paper = null, citations = [], authorsByPub = {} }) => ({
  id: 'openalex',
  getPublication: async () => paper,
  getContributions: async (pubId) => authorsByPub[pubId] ?? [],
  getCitations: async () => citations,
});
  ...
  describe('getCitationTree', () => {
    describe('when the publication exists', () => {
      ...
      it('returns the publication', () => {
        expect(tree.publication.pubId)
          .toBe('W1');
      });

      it('includes the cited contributions', () => {
        expect(tree.citedContributions)
          .toEqual([contribution('W1', 'A1', 1)]);
      });

      it('includes each citing publication in order', () => {
        expect(tree.citing.map(entry => entry.publication.pubId))
          .toEqual(['W2', 'W3']);
      });

      it('includes the contributions of each citing publication', () => {
        expect(tree.citing[0].contributions)
          .toEqual([contribution('W2', 'A1', 1)]);
      });

      it('includes the citations', () => {
        expect(tree.citations)
          .toEqual([publication('W2'), publication('W3')]);
      });
    });
